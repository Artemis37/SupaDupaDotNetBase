...
public class DeleteVehicleCommandHandlerTests
{
    ...
    [Test]
    public async Task Handle_SoftDeletesVehicle_WhenVehicleExistsAndOwned()
    {
        ...
        Assert.That(existingVehicle.IsDeleted, Is.True);
    ...
    public async Task Handle_ReturnsFailure_WhenVehicleNotOwned()
    {
        ...
        Assert.That(result.IsFailure, Is.True);
        Assert.That(result.Error, Is.EqualTo("Unauthorized to delete this vehicle"));
        _mockRepository.Verify(r => r.UpdateAsync(It.IsAny<Domain.Models.Vehicle>()), Times.Never);
        _mockRepository.Verify(r => r.SaveChangesAsync(), Times.Never);
    }

    [Test]
    public async Task Handle_SetsAuditFields_Correctly()
    {
        var existingVehicle = new Domain.Models.Vehicle
        {
            Id = 1,
